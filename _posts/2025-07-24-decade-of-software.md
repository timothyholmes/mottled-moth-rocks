...
<section class="caption">The cafe + fav lunch + walking path</section>
...
<ol class="footnotes">
    <li>
        Frontend vs Backend:
        <ul>
            <li>Frontend is the code that runs on your device, displaying an app's data and allowing you to interact with it. These are websites, mobile apps, etc. Some frontend tech mentioned in this post: HTML, CSS, Angular, React, Vue.</li>
            <li>The backend is the code that runs on the server of an app's owner, and processes that data at the user's behest. Associated technologies in this post are NodeJS, PHP, and Hapi.  Databases are also on the backend, of which SQL, DynamoDB, and MongoDB are examples.</li>
        </ul>
    </li>
    <li>
        Microservice architecture is a style of designing backends where individual areas of responsibility (profile interactions, photo management, follower and following tracking, etc.) are grouped together and work in isolation, in contrast to a monolith which puts all the code for the entire application together. It's a powerful design that offers a lot of flexibility in regards to scaling, but it can often be too complex for most problems. The complexity gives it a bad rap.
    </li>
    <li>
